...
//         console.log("🔹 Deleting Candidate:", role, "By:", userRole, "Access:", access, "Status:", status);
...
exports.deleteCandidate = async (req, res) => {
    try {
        ...
        // ❌ SuperAdmin cannot be deleted by anyone
        if (role === "SuperAdmin") {
            return res.status(403).json({ status: 403, message: "You cannot delete a SuperAdmin" });
        }
        ...
        if (!hasDeletePermission) {
            return res.status(403).json({ status: 403, message: "Access denied: You do not have permission to delete this role" });
        }
        ...
        console.log(`✅ ${role} Deleted Successfully by ${userRole}:`, deletedCandidate);
        res.status(200).json({ status: 200, message: `${role} deleted successfully`, data: deletedCandidate });

    } catch (error) {
        console.error("❌ Error Deleting Candidate:", error.message);
        res.status(500).json({ status: 500, message: "Error deleting candidate", error: error.message });
    }
};
